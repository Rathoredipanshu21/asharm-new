...
/*
================================================================================
 ASHRAM FAMILY MANAGEMENT - ALL-IN-ONE PAGE (REVISED V3 - With Ritwiks)
================================================================================
 This file handles:
 1. Database Connection.
 2. Adding Families with members, relations, and optional Ritwik assignments.
 3. A comprehensive Edit modal to update all family and member details.
 4. Deleting Families.
 5. Displaying all Families, their members, and Ritwik assignments.
*/
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add_family') {
        ...
        if (empty($head_name)) {
            ...
            try {
                ...
                // 2. Add members
                $sqlMember = "INSERT INTO family_members (family_id, name, relation_to_head, ritwik_id) VALUES (?, ?, ?, ?)";
                ...
                // Add Head of Family
                $stmtMember->execute([$family_id, $head_name, 'Head of Family', $head_ritwik_id]);
                ...
                for ($i = 1; $i < count($_POST['member_name']); $i++) {
                    ...
                    if (!empty($member_name) && !empty($relation)) {
                        $ritwik_id = $apply_ritwik_to_all ? $head_ritwik_id : (!empty($_POST['ritwik_id'][$i]) ? $_POST['ritwik_id'][$i] : null);
                        $stmtMember->execute([$family_id, $member_name, $relation, $ritwik_id]);
                    }
                }
    ...
    if ($action === 'update_family') {
        ...
        $pdo->beginTransaction();
        try {
            // 1. Delete members marked for deletion
            if (!empty($delete_member_ids)) {
                ...
                $params = array_merge($delete_member_ids, [$family_id]);
                $stmt->execute($params);
            }
            ...
            if (isset($_POST['member_name'])) {
                foreach ($_POST['member_name'] as $member_id => $member_name) {
                    ...
                    $stmt = $pdo->prepare("UPDATE family_members SET name = ?, relation_to_head = ?, ritwik_id = ? WHERE id = ?");
                    $stmt->execute([trim($member_name), trim($relation), $ritwik_id, $member_id]);
                }
            }
            
            // 3. Add new members
            if (isset($_POST['new_member_name'])) {
                $stmt = $pdo->prepare("INSERT INTO family_members (family_id, name, relation_to_head, ritwik_id) VALUES (?, ?, ?, ?)");
                for ($i = 0; $i < count($_POST['new_member_name']); $i++) {
                    ...
                    if (!empty($name) && !empty($relation)) {
                        ...
                        $stmt->execute([$family_id, $name, $relation, $ritwik_id]);
                    }
                }
            }

            // 4. Update the family's head_of_family_id
...
$sql_members = "SELECT fm.id, fm.family_id, fm.name, fm.relation_to_head, fm.ritwik_id, r.name as ritwik_name
                FROM family_members fm
                LEFT JOIN ritwiks r ON fm.ritwik_id = r.id";
$all_members = $pdo->query($sql_members)->fetchAll(PDO::FETCH_ASSOC);

// Fetch all Ritwiks to populate dropdowns
...
<body class="font-sans">
                        ...
                        <div class="mt-4 border-t pt-4">
                            ...
                            <ul class="space-y-2 text-gray-700">
                                ...
                                <?php else: foreach ($current_family_members as $member): ?>
                                    <li class="text-sm">
                                        <div class="font-semibold"><?= htmlspecialchars($member['name']) ?> <span class="text-gray-500 font-normal">- <?= htmlspecialchars($member['relation_to_head']) ?></span></div>
    ...
    <div id="addFamilyModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 opacity-0 pointer-events-none modal-backdrop">
        <div class="bg-white rounded-lg shadow-2xl w-full max-w-3xl p-8 modal-content transform scale-95 opacity-0">
            <div class="flex justify-between items-center mb-6"><h2 class="text-2xl font-bold text-gray-800">Add New Family</h2><button class="close-modal-btn text-gray-400 hover:text-gray-600 text-3xl">&times;</button></div>
            <form action="add_family.php" method="POST">
                ...
                <div class="space-y-4 max-h-[60vh] overflow-y-auto pr-4">
                    <p class="text-sm text-gray-600 bg-gray-100 p-3 rounded-lg"><i class="fas fa-info-circle mr-2"></i>The first member is the 'Head of Family'. Add other members with their relation and optional Ritwik.</p>
                    <div id="members-container">
                        <div class="member-entry grid grid-cols-12 gap-3 items-center mb-3">
                            <i class="fas fa-crown text-yellow-500 fa-lg col-span-1 text-center"></i>
                            <input type="text" name="member_name[]" placeholder="Head of Family Name" class="col-span-4 p-2 border rounded" required>
                            <input type="text" name="relation_to_head[]" value="Head of Family" class="col-span-3 p-2 bg-gray-100 border rounded" readonly>
                            <select name="ritwik_id[]" class="col-span-3 p-2 border rounded ritwik-select">
                                <option value="">Select Ritwik (Optional)</option>
                                <?php foreach($all_ritwiks as $ritwik): ?>
                                <option value="<?= $ritwik['id'] ?>"><?= htmlspecialchars($ritwik['name']) ?></option>
                                <?php endforeach; ?>
                            </select>
    ...
    <div id="editFamilyModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 opacity-0 pointer-events-none modal-backdrop">
        <div class="bg-white rounded-lg shadow-2xl w-full max-w-4xl p-8 modal-content transform scale-95 opacity-0">
            <div class="flex justify-between items-center mb-6">
                <h2 id="edit-modal-title" class="text-2xl font-bold text-gray-800">Edit Family</h2>
    ...
    <div id="deleteConfirmModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 opacity-0 pointer-events-none modal-backdrop">
        <div class="bg-white rounded-lg shadow-2xl w-full max-w-md p-8 modal-content transform scale-95 opacity-0 text-center">
            ...
            <form action="add_family.php" method="POST">
                ...
                <input type="hidden" id="delete-family-id" name="family_id">
    ...
    <script>
        ...
        // --- Helper to build Ritwik dropdown options ---
        function buildRitwikOptions(selectedValue = '') {
            let options = '<option value="">Select Ritwik (Optional)</option>';
        ...
        document.getElementById('add-member-btn').addEventListener('click', () => {
            ...
            newMemberEntry.className = 'member-entry grid grid-cols-12 gap-3 items-center mb-3';
            ...
            newMemberEntry.innerHTML = `
                <i class="fas fa-user text-gray-400 fa-lg col-span-1 text-center"></i>
                <input type="text" name="member_name[]" placeholder="Member Name" class="col-span-4 p-2 border rounded" required>
                <input type="text" name="relation_to_head[]" placeholder="Relation (e.g., Wife)" class="col-span-3 p-2 border rounded" required>
                <select name="ritwik_id[]" class="col-span-3 p-2 border rounded ritwik-select">${buildRitwikOptions(selectedRitwik)}</select>
                <button type="button" class="remove-member-btn text-red-500 hover:text-red-700 col-span-1"><i class="fas fa-trash"></i></button>
            `;
        ...
        document.querySelectorAll('.edit-family-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                let formContent = `
                    ...
                    <div id="existing-members-list" class="space-y-3">
                        <label class="font-semibold text-gray-700">Existing Members</label>
                `;
                
                members.forEach(member => {
                    const isHead = member.id == family.head_of_family_id;
                    formContent += `
                        <div class="member-edit-row grid grid-cols-12 gap-2 items-center" data-member-id="${member.id}">
                            <div class="col-span-1 text-center">
                                <input type="radio" name="head_of_family_id" value="${member.id}" class="h-4 w-4" ${isHead ? 'checked' : ''}>
                            </div>
                            <input type="text" name="member_name[${member.id}]" value="${member.name}" class="col-span-3 p-2 border rounded">
                            <input type="text" name="member_relation[${member.id}]" value="${member.relation_to_head}" class="col-span-3 p-2 border rounded member-relation-input" ${isHead ? 'readonly' : ''}>
                            <select name="member_ritwik_id[${member.id}]" class="col-span-4 p-2 border rounded">${buildRitwikOptions(member.ritwik_id)}</select>
                            <button type="button" class="remove-member-btn text-red-500 hover:text-red-700 col-span-1"><i class="fas fa-trash"></i></button>
                        </div>`;
                });

                formContent += `</div>
                    <div id="new-members-container" class="mt-6 pt-4 border-t space-y-3"></div>
                    <button type="button" id="add-new-member-in-modal-btn" class="w-full mt-2 border-2 border-dashed border-gray-300 text-gray-500 hover:bg-gray-100 rounded-lg py-2 transition">
                        <i class="fas fa-plus mr-2"></i>Add New Member to Family
                    </button>
                `;
        ...
        editFamilyModal.addEventListener('click', e => {
            ...
            if (removeBtn) {
                ...
                const hiddenInput = `<input type="hidden" name="delete_member_ids[]" value="${row.dataset.memberId}">`;
                editFormContainer.insertAdjacentHTML('beforeend', hiddenInput);
            }

            if (e.target.closest('#add-new-member-in-modal-btn')) {
                const container = document.getElementById('new-members-container');
                const newRow = `
                    <div class="new-member-row grid grid-cols-12 gap-2 items-center">
                        <i class="fas fa-user-plus text-gray-400 col-span-1 text-center"></i>
                        <input type="text" name="new_member_name[]" placeholder="New Member Name" class="col-span-3 p-2 border rounded" required>
                        <input type="text" name="new_member_relation[]" placeholder="Relation" class="col-span-3 p-2 border rounded" required>
                        <select name="new_member_ritwik_id[]" class="col-span-4 p-2 border rounded">${buildRitwikOptions()}</select>
                        <button type="button" class="remove-new-row-btn text-red-500 col-span-1"><i class="fas fa-times-circle"></i></button>
                    </div>
                `;
